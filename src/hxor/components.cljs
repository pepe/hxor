...
(hx/defnc Buttons [{:keys [kind store]}]
  [:div
   (for [k    ["nice" "leet"]
         :let [current (= kind k)]]
     (hiccup/parse [:button {:key (str (when current "disabled-" )kind)
                             :disabled current
...
(hx/defnc Counting [{:keys [count store]}]
  [:div
   [:label "HXOR must count to: "
    [:input {:value count
             :on-change #(ptk/emit! store (events/->SetCount (-> % .-target .-value Number.parseInt)))}]]
   [:div (map #(hiccup/parse [:span {:key (str %)} % " "]) (range 1 (inc count)))]])
